...
public class CloudNativeFastSchemaEvolutionV2Test extends StarRocksTestBase {
    ...
    @Test
    public void testCreateTablePropertyScenarios() throws Exception {
        LakeTable defaultTable = createLakeTable(uniqueName("create_default"), null);
        assertTableProperty(defaultTable.getName(), defaultTable, "true");

        LakeTable explicitTrue = createLakeTable(uniqueName("create_true"), "true");
        assertTableProperty(explicitTrue.getName(), explicitTrue, "true");

        LakeTable explicitFalse = createLakeTable(uniqueName("create_false"), "false");
        assertTableProperty(explicitFalse.getName(), explicitFalse, "false");
    ...
    public void testAlterTablePropertyScenarios() throws Exception {
        ...
        Assertions.assertTrue(job.isDisableCloudNativeFastSchemaEvolutionV2());
        assertJobSchemaMatchesSnapshot(job, originalSchema);
        assertTableSchemaMatchesSnapshot(table, originalSchema);
        assertTableProperty(table.getName(), table, "false");
        ...
        // invalid alter value
        String tableNameForInvalid = table.getName();
        Assertions.assertThrows(SemanticException.class,
                () -> alterTableProperty(tableNameForInvalid, "maybe"));
    }

    @Test
    public void testPropertyRestoredAfterFeRestart() throws Exception {
        LakeTable table = createLakeTable(uniqueName("restore_property"), null);
        Map<Long, SchemaInfo> schemaSnapshot = snapshotTableSchema(table);
        assertTableProperty(table.getName(), table, "true");

        UtFrameUtils.PseudoJournalReplayer.resetFollowerJournalQueue();
        UtFrameUtils.PseudoImage preAlterImage = new UtFrameUtils.PseudoImage();
        GlobalStateMgr.getCurrentState().getLocalMetastore().save(preAlterImage.getImageWriter());

        alterTableProperty(table.getName(), "false");
        LakeTableAsyncFastSchemaChangeJob job = waitForAsyncSchemaChangeJob(table);
        assertJobSchemaMatchesSnapshot(job, schemaSnapshot);
        LakeTable updatedTable = getLakeTable(table.getName());
        assertTableSchemaMatchesSnapshot(updatedTable, schemaSnapshot);
        assertTableProperty(updatedTable.getName(), updatedTable, "false");
        ...
        LocalMetastore restoredMetastore = new LocalMetastore(GlobalStateMgr.getCurrentState(), null, null);
        restoredMetastore.load(finalImage.getMetaBlockReader());
        Database restoredDb = restoredMetastore.getDb(DB_NAME);
        Assertions.assertNotNull(restoredDb, "Restored database not found");
        LakeTable restoredTable = (LakeTable) restoredMetastore.getTable(restoredDb.getFullName(), updatedTable.getName());
        ...
        Assertions.assertEquals("false", restoredTable.getTableProperty().getProperties()
        ...
        followerMetastore.load(preAlterImage.getMetaBlockReader());
        ModifyTablePropertyOperationLog log = (ModifyTablePropertyOperationLog)
                UtFrameUtils.PseudoJournalReplayer.replayNextJournal(OperationType.OP_ALTER_TABLE_PROPERTIES);
        followerMetastore.replayModifyTableProperty(OperationType.OP_ALTER_TABLE_PROPERTIES, log);
        Database followerDb = followerMetastore.getDb(DB_NAME);
        Assertions.assertNotNull(followerDb, "Follower database not found");
        LakeTable replayedTable = (LakeTable) followerMetastore.getTable(followerDb.getFullName(), table.getName());
        Assertions.assertNotNull(replayedTable, "Replayed table not found");
        Assertions.assertFalse(replayedTable.isFastSchemaEvolutionV2());
        Assertions.assertEquals("false", replayedTable.getTableProperty().getProperties()
                .get(PropertyAnalyzer.PROPERTIES_CLOUD_NATIVE_FAST_SCHEMA_EVOLUTION_V2));
        assertTableSchemaMatchesSnapshot(replayedTable, schemaSnapshot);
    }

    private static void assertTableProperty(String tableName, LakeTable table, String expectedValue) throws Exception {
        ...
        Assertions.assertEquals(expectedValue, table.getTableProperty().getProperties()
                .get(PropertyAnalyzer.PROPERTIES_CLOUD_NATIVE_FAST_SCHEMA_EVOLUTION_V2));
        String showStmt = showCreateTable(tableName);
