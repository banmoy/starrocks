...
public class IsomorphicLoadTest extends GroupCommitTestBase {
    ...
    private IsomorphicLoad load;

    @Before
    public void setup() throws Exception {
        executor = new TestThreadPoolExecutor();
        parallel = 4;
        assertTrue(parallel < allNodes.size());
        Map<String, String> map = new HashMap<>();
        map.put(StreamLoadHttpHeader.HTTP_FORMAT, "json");
        map.put(StreamLoadHttpHeader.HTTP_ENABLE_GROUP_COMMIT, "true");
        map.put(StreamLoadHttpHeader.HTTP_GROUP_COMMIT_ASYNC, "true");
        StreamLoadKvParams params = new StreamLoadKvParams(map);
        StreamLoadInfo streamLoadInfo =
                StreamLoadInfo.fromHttpStreamLoadRequest(null, -1, params);
        load = new IsomorphicLoad(
                1,
    ...
    public void testRequestBackendsSuccess() {
        ...
        assertSame(nodes, requestResult.getResult());
    }
    ...
    public void testRequestLoadFromCoordinatorBackend() throws Exception {
        ...
        String label = result1.getResult();
        assertNotNull(label);
        assertEquals(1, load.numRunningExecutors());
        GroupCommitLoadExecutor loadExecutor = load.getLoadExecutor(label);
        ...
        assertEquals(label, result2.getResult());
        ...
        assertEquals(0, load.numRunningExecutors());
    }
    ...
    public void testRequestLoadFromNoneCoordinatorBackend() throws Exception {
        ...
        RequestLoadResult result1 = load.requestLoad(nodes.get(0).getId(), nodes.get(0).getHost());
        assertTrue(result1.isOk());
        String label1 = result1.getResult();
        assertNotNull(label1);
        assertEquals(1, load.numRunningExecutors());
        GroupCommitLoadExecutor loadExecutor1 = load.getLoadExecutor(label1);
        ...
        assertTrue(result2.isOk());
        String label2 = result2.getResult();
        assertNotNull(label2);
        assertEquals(2, load.numRunningExecutors());
        assertNotEquals(label1, label2);
        GroupCommitLoadExecutor loadExecutor2 = load.getLoadExecutor(label2);
    ...
    private class TestThreadPoolExecutor implements Executor {
        ...
        public void manualRun(Runnable runnable) throws Exception {
            ...
            if (!(runnable instanceof GroupCommitLoadExecutor)) {
                runnable.run();
                return;
            }

            GroupCommitLoadExecutor loadExecutor = (GroupCommitLoadExecutor) runnable;
