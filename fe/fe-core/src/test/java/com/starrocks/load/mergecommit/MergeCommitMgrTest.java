...
public class MergeCommitMgrTest extends BatchWriteTestBase {

    private MergeCommitMgr mergeCommitMgr;
    ...
    @Before
    public void setup() {
        mergeCommitMgr = new MergeCommitMgr();
    ...
    public void testRequestBackends() {
        StreamLoadKvParams params1 = new StreamLoadKvParams(new HashMap<>() {{
                put(HTTP_MERGE_COMMIT_INTERVAL_MS, "1000");
                put(HTTP_MERGE_COMMIT_PARALLEL, "1");
            }});
        RequestCoordinatorBackendResult result1 =
                mergeCommitMgr.requestCoordinatorBackends(tableId1, params1);
        assertTrue(result1.isOk());
        assertEquals(1, result1.getValue().size());
        assertEquals(1, mergeCommitMgr.numMergeCommit());
        ...
        RequestCoordinatorBackendResult result2 =
                mergeCommitMgr.requestCoordinatorBackends(tableId1, params2);
        assertTrue(result2.isOk());
        assertEquals(1, result2.getValue().size());
        assertEquals(2, mergeCommitMgr.numMergeCommit());

        StreamLoadKvParams params3 = new StreamLoadKvParams(new HashMap<>() {{
                put(HTTP_MERGE_COMMIT_INTERVAL_MS, "10000");
                put(HTTP_MERGE_COMMIT_PARALLEL, "4");
            }});
        RequestCoordinatorBackendResult result3 =
                mergeCommitMgr.requestCoordinatorBackends(tableId2, params3);
        assertTrue(result3.isOk());
        assertEquals(4, result3.getValue().size());
        assertEquals(3, mergeCommitMgr.numMergeCommit());
        ...
        RequestCoordinatorBackendResult result4 =
                mergeCommitMgr.requestCoordinatorBackends(tableId3, params4);
        assertTrue(result4.isOk());
        assertEquals(4, result4.getValue().size());
        assertEquals(4, mergeCommitMgr.numMergeCommit());
    }

    @Test
    public void testRequestLoad() {
        StreamLoadKvParams params = new StreamLoadKvParams(new HashMap<>() {{
                put(HTTP_MERGE_COMMIT_INTERVAL_MS, "100000");
                put(HTTP_MERGE_COMMIT_PARALLEL, "4");
            }});
        RequestLoadResult result1 = mergeCommitMgr.requestLoad(
                tableId4, params, allNodes.get(0).getId(), allNodes.get(0).getHost());
        assertTrue(result1.isOk());
        assertNotNull(result1.getValue());
        assertEquals(1, mergeCommitMgr.numMergeCommit());

        RequestLoadResult result2 = mergeCommitMgr.requestLoad(
                tableId4, params, allNodes.get(0).getId(), allNodes.get(0).getHost());
        assertTrue(result2.isOk());
    ...
    public void testCheckParameters() {
        ...
        assertEquals(0, mergeCommitMgr.numMergeCommit());

        StreamLoadKvParams params2 = new StreamLoadKvParams(new HashMap<>() {{
                put(HTTP_MERGE_COMMIT_INTERVAL_MS, "10000");
            }});
        RequestCoordinatorBackendResult result2 =
                mergeCommitMgr.requestCoordinatorBackends(tableId2, params2);
        ...
        RequestCoordinatorBackendResult result3 =
                mergeCommitMgr.requestCoordinatorBackends(tableId3, params3);
    ...
    @Test
    public void testCleanupInactiveMergeCommit() {
        StreamLoadKvParams params1 = new StreamLoadKvParams(new HashMap<>() {{
                put(HTTP_MERGE_COMMIT_INTERVAL_MS, "10000");
                put(HTTP_MERGE_COMMIT_PARALLEL, "4");
            }});
        RequestCoordinatorBackendResult result1 = mergeCommitMgr.requestCoordinatorBackends(tableId1, params1);
        assertTrue(result1.isOk());
        assertEquals(1, mergeCommitMgr.numMergeCommit());
        ...
        new MockUp<IsomorphicMergeCommit>() {
        ...
        mergeCommitMgr.cleanupInactiveMergeCommit();
        assertEquals(0, mergeCommitMgr.numMergeCommit());
    }
    ...
    public void testAssignerRegisterBatchWriteFail() {
        ...
        CoordinatorBackendAssigner assigner = mergeCommitMgr.getCoordinatorBackendAssigner();
        new Expectations(assigner) {
            {
                assigner.registerMergeCommit(anyLong, anyLong, (TableId) any, anyInt);
                result = new Exception("registerBatchWrite failed");
            }
        };
        RequestCoordinatorBackendResult result = mergeCommitMgr.requestCoordinatorBackends(tableId1, params1);
