...
public class TableSchemaService {
    ...
    public static TGetTableSchemaResponse getTableSchema(TGetTableSchemaRequest request) {
        ...
        long schemaId = request.getSchema_id();
        ...
        try {
            ...
            if (table == null) {
                ...
                status.addToError_msgs("table not found in catalog, maybe the table is dropped");
            ...
            if (!table.isNativeTableOrMaterializedView()) {
                ...
                status.addToError_msgs("schema service not support table type: " + table.getType());
                response.setStatus(status);
                return response;
            }
            ...
            if (schemaInfo.isPresent()) {
                ...
                if (txnState == null) {
                    ...
                    status.addToError_msgs(String.format("transaction %s not found, maybe has been cleaned", txnId));
                } else if (txnState.getTransactionStatus().isFinalStatus()) {
                    status.setStatus_code(TStatusCode.NOT_FOUND);
                    status.addToError_msgs("transaction has finished, status: "
                            + txnState.getTransactionStatus().toString());
                } else {
                    status.setStatus_code(TStatusCode.INTERNAL_ERROR);
                    status.addToError_msgs("schema not found both in catalog and history schemas " +
                            "which should not happen, please contact developers.");
                }
                response.setStatus(status);
            }
        } catch (Exception e) {
            TStatus status = new TStatus(TStatusCode.INTERNAL_ERROR);
            status.addToError_msgs(String.format("exception happened when getting table schema, error: %s", e.getMessage()));
            response.setStatus(status);
        }
        return response;
    }
    ...
    private static SchemaStatusOr findSchemaInQueryCoordinator(long schemaId, TUniqueId queryId) {
        ...
        if (coordinator == null) {
            return SchemaStatusOr.error(TStatusCode.QUERY_NOT_EXISTS,
                    "query not found, maybe the query has finished");
        }
        ...
        return schemaInfo.map(SchemaStatusOr::value).orElseGet(() -> SchemaStatusOr.error(TStatusCode.INTERNAL_ERROR,
                "schema not found in query plan which should not happen, please contact developers"));
    }
    ...
    private static Status validateParameters(TGetTableSchemaRequest request) {
        try {
            Preconditions.checkArgument(request.isSetSchema_id(), "schema id not set");
            Preconditions.checkArgument(request.isSetDb_id(), "db id not set");
            Preconditions.checkArgument(request.isSetTable_id(), "table id not set");
            ...
            switch (requestSource) {
                ...
                default -> throw new UnsupportedOperationException("unsupported request source: " + requestSource.name());
            }
        } catch (Exception e) {
