...
public class TransactionLoadCoordinatorMgr {
    ...
    public @NonNull ComputeNode get(String label, String dbName) throws StarRocksException {
        ...
        return nodeId != null ? getNodeFromId(nodeId) : getNodeFromTransactionState(dbName, label);
    }
     ...
     * @throws StarRocksException if the node cannot be found
     */
    private @NonNull ComputeNode getNodeFromId(Long nodeId) throws StarRocksException {
    ...
    private @NonNull ComputeNode getNodeFromTransactionState(String label, String dbName) throws StarRocksException {
        ...
        if (state == null) {
            throw new StarRocksException(String.format("Can't find the transaction with db [%s] and labe[%s]." +
                    " The possible reasons: 1. FE leader is started or switched; 2. The transaction timeouts and is cleaned.", dbName, label));
        }
        ...
        if (TransactionState.TxnSourceType.BE.equals(txnCoordinator.sourceType)) {
            throw new StarRocksException(String.format("The source type for transaction stream load with db [%s] and label[%s] should be %s, but is %s.",
