...
Status BinlogReader::init() {
    VectorizedSchema& schema = _reader_params.output_schema;
    ...
    _data_schema = VectorizedSchema(&_reader_params.output_schema, _data_column_index);
...
Status BinlogReader::_seek_binlog_file_reader(int64_t version, int64_t seq_id) {
    ...
    std::string file_path = BinlogUtil::binlog_file_path(_binlog_manager->storage_path(), _file_meta->id());
...
void BinlogReader::close() {
    ...
    _closed = true;
    _reset();
    _binlog_manager->release_reader(_reader_id);
}
