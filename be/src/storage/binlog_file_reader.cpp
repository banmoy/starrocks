...
Status BinlogFileReader::scan_pages_to_load(int64_t file_id, RandomAccessFile* read_file, int64_t file_size,
    ...
    while (true) {
        ...
        // TODO for duplicate key, there must be only one rowset used a page, and it's version is
        //  same as the page_header->version(), but for primary key, need to check PageHeaderPB#rowsets
        if (!filter.is_valid_rowset(page_header->version())) {
            break;
        }
...
StatusOr<std::shared_ptr<BinlogFileMetaPB>> BinlogFileReader::load(int64_t file_id, std::string& file_path,
    ...
    if (!st.ok()) {
        ...
        if (filter.is_valid_seq(file_meta->end_version(), file_meta->end_seq_id())) {
            // because data in a binlog file is appended, so if the last rowset(file_meta->end_version())
            // is valid, the previous rowsets must be valid
            if (filter.is_valid_rowset(file_meta->end_version())) {
                return file_meta;
            }
        }
    ...
    RETURN_IF_ERROR(scan_pages_to_load(file_id, read_file.get(), file_size, file_meta.get(), filter));
    return file_meta;
}
