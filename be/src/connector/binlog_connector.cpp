...
BinlogMetaFieldMap BinlogDataSource::_build_binlog_meta_fields(ColumnId start_cid) {
    ...
    VectorizedFieldPtr op_field =
            std::make_shared<VectorizedField>(cid, g_PlanNodes_constants.BINLOG_OP_COLUMN_NAME,
    ...
    VectorizedFieldPtr version_field =
            std::make_shared<VectorizedField>(cid, g_PlanNodes_constants.BINLOG_VERSION_COLUMN_NAME,
    ...
    VectorizedFieldPtr seq_id_field =
            std::make_shared<VectorizedField>(cid, g_PlanNodes_constants.BINLOG_SEQ_ID_COLUMN_NAME,
                                              get_type_info(TYPE_BIGINT), STORAGE_AGGREGATE_NONE, 8, false, false);
    fields.emplace(g_PlanNodes_constants.BINLOG_SEQ_ID_COLUMN_NAME, seq_id_field);
    cid += 1;

    VectorizedFieldPtr timestamp_field =
            std::make_shared<VectorizedField>(cid, g_PlanNodes_constants.BINLOG_TIMESTAMP_COLUMN_NAME,
                                              get_type_info(TYPE_BIGINT), STORAGE_AGGREGATE_NONE, 8, false, false);
