        ...
        for (; iter2 != iter->second.end(); iter2++) {
            for (const auto& scan_range : iter2->second) {
                ...
                RETURN_IF_ERROR(exec_env->stream_context_mgr()->create_channel_context(
                        ...
                        active_time_ms));
                ctx->fragment_instance_id = unique_request.params.fragment_instance_id;
                down_cast<StreamLoadPipe*>(ctx->body_sink.get())->set_context(txn_id, label, ctx->fragment_instance_id);
                DeferOp op([&] {
                    if (ctx->unref()) {
                        delete ctx;
                    }
