...
TEST_F(BinlogManagerTest, test_ingestion_commit) {
    ...
    for (int64_t version = 1, next_file_id = 0; version < 1000; version++) {
        ...
        ASSERT_EQ(next_file_id, lsn_map.size());
        ASSERT_EQ(version, rowset_count_map.size());
        ...
        if (last_file_meta != nullptr) {
            ...
            file_meta->set_id(last_file_meta->id());
            file_meta->set_start_version(last_file_meta->start_version());
            file_meta->set_start_seq_id(last_file_meta->start_seq_id());
            file_meta->set_file_size(last_file_meta->id() * version + 1);
            for (auto rs : last_file_meta->rowsets()) {
                file_meta->add_rowsets(rs);
            }
            file_meta->add_rowsets(version);
            result->metas.push_back(file_meta);
            result->next_file_id = param->start_file_id + (std::rand() % 3);
            expect_file_metas[last_file_meta->id()] = file_meta;
            version_to_file_ids[version].emplace(file_meta->id());
        } else {
        ...
        int64_t expect_binlog_file_size;
...
TEST_F(BinlogManagerTest, test_ingestion_abort) {
    ...
    int64_t file_id = result_1->next_file_id - 1;
    std::string path = BinlogUtil::binlog_file_path(_binlog_file_dir, result_1->next_file_id - 1);
