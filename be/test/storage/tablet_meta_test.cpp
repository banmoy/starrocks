...
TEST(TabletMetaTest, test_create) {
    ...
    ASSERT_EQ(5, binlog_config_ptr->binlog_ttl_second);
    ASSERT_EQ(5, binlog_config_ptr->binlog_max_size);
}
...
TEST(TabletMetaTest, test_init_from_pb) {
    ...
    std::shared_ptr<BinlogConfig> binlog_config_ptr1 = tablet_meta->get_binlog_config();
    ASSERT_TRUE(binlog_config_ptr == nullptr);

    tablet_meta->init_from_pb(&tablet_meta_pb);
    ASSERT_EQ(3, binlog_config_ptr1->version);
    ASSERT_TRUE(binlog_config_ptr1->binlog_enable);
    ASSERT_EQ(823, binlog_config_ptr1->binlog_ttl_second);
    ASSERT_EQ(984, binlog_config_ptr1->binlog_max_size);
}
