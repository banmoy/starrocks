...
protected:
    ...
    void test_reader(VectorizedSchema& output_schema, OutputVerifier& verifier);
...
void verify_binlog_reader(std::shared_ptr<BinlogManager> binlog_manager, VectorizedSchema& output_schema,
    ...
    StatusOr<std::shared_ptr<BinlogReader>> status_or = binlog_manager->create_reader(params);
    ASSERT_TRUE(status_or.ok());
    std::shared_ptr<BinlogReader> binlog_reader = status_or.value();
    ...
    while (num_rows < version_infos.back().culmulitive_num_rows) {
        ...
        for (int32_t i = 0; i < chunk->num_rows(); i++) {
            ...
            DatumTuple actual_row = chunk->get(i);
...
void BinlogReaderTest::test_reader(VectorizedSchema& output_schema, OutputVerifier& verifier) {
    RowsetId rowset_id;
    int32_t start_key = 0;
...
TEST_F(BinlogReaderTest, test_read_full_columns) {
    ...
    VectorizedFieldPtr op = make_field(4, BINLOG_OP, TYPE_TINYINT);
    VectorizedFieldPtr version = make_field(5, BINLOG_VERSION, TYPE_BIGINT);
    VectorizedFieldPtr seq_id = make_field(6, BINLOG_SEQ_ID, TYPE_BIGINT);
    VectorizedFieldPtr timestamp = make_field(7, BINLOG_TIMESTAMP, TYPE_BIGINT);
