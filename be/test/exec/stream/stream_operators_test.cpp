...
std::shared_ptr<TPlanNode> StreamOperatorsTest::_create_tplan_node(int node_id, int tuple_id) {
    // TODO what's the tuple descriptor in StreamOperatorsTest
    ...
    stream_scan_node.__set_source_type(StreamSourceType::BINLOG);

    TBinlogScanNode binlog_scan_node;
    binlog_scan_node.__set_tuple_id(tuple_id);
    stream_scan_node.__set_binlog_scan(binlog_scan_node);
...
TEST_F(StreamOperatorsTest, binlog_dop_1) {
    // TODO
    // 1. BinlogConnector mock data same as GeneratorStreamSourceOperator::pull_chunk()
    // 2. BinlogConnector can get BinlogOffset for a tablet which set in StreamPipelineTest::StartEpoch()
    DCHECK_IF_ERROR(StartMV([&]() {
        ...
        _pipeline_builder = [&](RuntimeState* state) {
            ...
            pipeline::PipelineBuilderContext* pipeline_context =
                    _obj_pool->add(new pipeline::PipelineBuilderContext(_fragment_ctx, _degree_of_parallelism));
            OpFactories op_factories = binlog_scan_node->decompose_to_pipeline(pipeline_context);
